...
describe("/api/stepfunctions/executions", () => {
  ...
  beforeEach(() => {
    mockSend = vi.fn().mockResolvedValue({
      executions: [
        {
          executionArn:
            "arn:aws:states:us-east-1:123456789012:execution:test-state-machine:execution-1",
          name: "execution-1",
          status: "SUCCEEDED",
        ...
        {
          executionArn:
            "arn:aws:states:us-east-1:123456789012:execution:test-state-machine:execution-2",
          name: "execution-2",
          status: "RUNNING",
  ...
  it("filters executions by status", async () => {
    ...
    const request = new Request(
      "http://localhost:3000/api/stepfunctions/executions?status=RUNNING"
    );
  ...
  it("validates and limits maxResults parameter", async () => {
    ...
    const request1 = new Request(
      "http://localhost:3000/api/stepfunctions/executions?maxResults=50"
    );
    await GET(request1);
    ...
    const request2 = new Request(
      "http://localhost:3000/api/stepfunctions/executions?maxResults=200"
    );
    await GET(request2);
  ...
  it("handles invalid maxResults gracefully", async () => {
    ...
    const request = new Request(
      "http://localhost:3000/api/stepfunctions/executions?maxResults=invalid"
    );
  ...
  it("passes nextToken for pagination", async () => {
    ...
    const request = new Request(
      "http://localhost:3000/api/stepfunctions/executions?nextToken=page-2-token"
    );
  ...
  it("combines multiple query parameters correctly", async () => {
    ...
    const request = new Request(
      "http://localhost:3000/api/stepfunctions/executions?status=FAILED&maxResults=25&nextToken=token123"
    );
