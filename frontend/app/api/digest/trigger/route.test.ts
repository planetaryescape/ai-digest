...
describe('/api/digest/trigger', () => {
  ...
  beforeEach(() => {
    mockLambdaSend = vi.fn().mockResolvedValue({
      StatusCode: 200,
      Payload: new TextEncoder().encode(JSON.stringify({ success: true, message: 'Digest generated' })),
  ...
  it('includes all payload fields correctly', async () => {
    ...
    expect(new Date(payload.timestamp).getTime()).toBeGreaterThanOrEqual(new Date(beforeTime).getTime());
    expect(new Date(payload.timestamp).getTime()).toBeLessThanOrEqual(new Date(afterTime).getTime());
  });
