...
describe('Authentication Security Tests', () => {
  beforeEach(() => {
    process.env.STEP_FUNCTIONS_STATE_MACHINE_ARN = 'arn:aws:states:us-east-1:123456789012:stateMachine:test';
  ...
  describe('Authentication Bypass Attempts', () => {
    ...
    it('blocks unauthenticated access to /api/stepfunctions/status', async () => {
      ...
      const request = new Request('http://localhost:3000/api/stepfunctions/status?executionArn=test');
  ...
  describe('Authorization Checks', () => {
    it('does not expose sensitive information in error messages', async () => {
      ...
      vi.mocked(SFNClient).mockImplementation(() => ({
        send: vi.fn().mockRejectedValue(new Error('User arn:aws:iam::123456789012:user/admin does not have permission')),
      } as any));
    ...
    it('prevents execution ARN injection in status endpoint', async () => {
      ...
      const request = new Request(`http://localhost:3000/api/stepfunctions/status?executionArn=${encodeURIComponent(maliciousArn)}`);
    ...
    it('validates and sanitizes input parameters', async () => {
      ...
      const request = new Request('http://localhost:3000/api/stepfunctions/executions?status=INVALID_STATUS&maxResults=999999');
  ...
  describe('Rate Limiting and DoS Prevention', () => {
    ...
    it('validates maxResults to prevent resource exhaustion', async () => {
      ...
      const request = new Request('http://localhost:3000/api/stepfunctions/executions?maxResults=10000');
