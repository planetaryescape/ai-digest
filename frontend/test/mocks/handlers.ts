...
export const handlers = [
  ...
  http.get("/api/stepfunctions/executions", ({ request }) => {
    const url = new URL(request.url);
    const maxResults = Number.parseInt(url.searchParams.get("maxResults") || "10", 10);

    return HttpResponse.json({
      executions: [
        {
          executionArn:
            "arn:aws:states:us-east-1:123456789012:execution:test-state-machine:test-execution-1",
        ...
        {
          executionArn:
            "arn:aws:states:us-east-1:123456789012:execution:test-state-machine:test-execution-2",
