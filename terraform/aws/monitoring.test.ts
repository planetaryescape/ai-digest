...
describe('Terraform Monitoring Configuration', () => {
  ...
  beforeAll(async () => {
    ...
    const exists = await fs.access(monitoringFile).then(() => true).catch(() => false);
  ...
  describe('CloudWatch Alarms', () => {
    ...
    it('should define Lambda throttle alarm', () => {
      expect(monitoringConfig).toContain('resource "aws_cloudwatch_metric_alarm" "lambda_throttles"');
    ...
    it('should define Lambda duration alarm', () => {
      expect(monitoringConfig).toContain('resource "aws_cloudwatch_metric_alarm" "lambda_duration"');
    ...
    it('should define API Gateway 4XX alarm', () => {
      expect(monitoringConfig).toContain('resource "aws_cloudwatch_metric_alarm" "api_gateway_4xx"');
    ...
    it('should define API Gateway 5XX alarm', () => {
      expect(monitoringConfig).toContain('resource "aws_cloudwatch_metric_alarm" "api_gateway_5xx"');
      expect(monitoringConfig).toContain('metric_name         = "5XXError"');
      expect(monitoringConfig).toContain('threshold           = "5"');
    });

    it('should define DynamoDB error alarm', () => {
      expect(monitoringConfig).toContain('resource "aws_cloudwatch_metric_alarm" "dynamodb_errors"');
  ...
  describe('S3 Metrics Configuration', () => {
    ...
    it('should define S3 bucket metrics filter', () => {
      expect(monitoringConfig).toContain('resource "aws_s3_bucket_metric" "processed_emails_metrics"');
    ...
    it('should pass terraform validation', async () => {
      try {
        ...
        if (error.message.includes('terraform: command not found')) {
          console.warn('Terraform not installed, skipping validation test');
        } else {
          throw error;
        }
      }
